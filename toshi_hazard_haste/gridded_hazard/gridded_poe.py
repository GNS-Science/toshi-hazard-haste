...
def trim_poes(min_poe: float, max_poe: float, ground_accels: Iterable[float], annuals_poes: Iterable[float]):
    acc_result = []
    poe_result = []
    for a, p in zip(ground_accels, annuals_poes):
...
def compute_hazard_at_poe(
    ...
    if not np.all(np.diff(xp) >= 0):  # raise is x_accel_levels not increasing or at least not dropping,
        raise ValueError('poe values not monotone.')
